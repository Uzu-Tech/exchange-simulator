...
public:
    MappedDatasetFile(const fs::path& file_path, size_t num_ticks, size_t max_logs_per_tick = 1)
        : file_desc(file_path) {
        max_logs = num_ticks * max_logs_per_tick;
...
struct RunDirectory {
    fs::path path;
    RunDirectory(const fs::path& log_dir, const fs::path& config_path) {
        fs::create_directories(log_dir);
        // Create run directory
        path = log_dir / get_timestamp();
        fs::create_directory(path);
        // Create symlink
        fs::path link = log_dir / "latest";
        std::error_code ec;
        fs::remove(link, ec); // important: remove old symlink safely
        fs::create_symlink(path.filename(), link);
        // Save config file
        fs::copy_file(config_path, path / config_path.filename());
    }
};

class Logger {
public:
    Logger(const fs::path& log_dir, const fs::path& config_path, size_t num_ticks)
        : run_dir(log_dir, config_path),
